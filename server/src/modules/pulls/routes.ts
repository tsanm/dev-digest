...
export default async function pullsRoutes(appBase: FastifyInstance) {
  ...
  app.get('/repos/:id/pulls', { schema: { params: IdParams } }, async (req): Promise<PrMeta[]> => {
    ...
    // so a couple of IN-queries + JS grouping are cheap.
    ...
    if (prIds.length > 0) {
      ...
      // Roll up findings-by-severity for those latest reviews (one IN-query).
      ...
      if (latestIds.length > 0) {
        ...
        for (const f of findingRows) {
          const arr = byReview.get(f.reviewId) ?? [];
          arr.push({ severity: f.severity });
          byReview.set(f.reviewId, arr);
        }
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        // Per-severity finding counts of the latest review (null until reviewed).
